...
public class GrowthOfAPopulation {
    public static int nbYear(int p0, double percent, int aug, int p) {
        ...
        // Continue until the population reaches or exceeds the target
        while (p0 < p) {
            // Calculate the new population: add percent growth and aug immigrants
            p0 = (int) (p0 + (p0 * (percent / 100)) + aug);

            // Increment year after population update
            year++;
        }
    ...
    public static int nbYear0(int p0, double percent, int aug, int p) {
        // IntStream.iterate to simulate the yearly growth
        return (int) IntStream.iterate(p0, population -> (int) (population + (population * percent / 100) + aug))
                .takeWhile(population -> population < p) // continue while population < target
                .count(); // count how many iterations (years) were needed
    }

    //Test case with streams
    public static void main(String[] args) {
        System.out.println(nbYear0(1000, 2, 50, 1200)); // Output: 3
        System.out.println(nbYear0(1500, 5, 100, 5000)); // Output: 15
        System.out.println(nbYear0(1500000, 2.5, 10000, 2000000)); // Output: 10
    }
}
