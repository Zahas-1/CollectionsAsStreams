...
Write a function which computes the depth of its integer argument.
...
public class IntegerDepth {
    public int computeDepth(int n) {
        Set<Character> digitsSeen = new HashSet<>();  // Store unique digits as characters
        int multiplier = 0;

        // Keep going until we've seen all 10 digits (0-9)
        while (digitsSeen.size() < 10) {
            multiplier++;
            int current = n * multiplier;

            // Add each digit from the current number to the set
            for (char digit : String.valueOf(current).toCharArray()) {
                digitsSeen.add(digit);
            }

            // Optional: Logging each step
            System.out.println("n * " + multiplier + " = " + current + " | digits seen: " + digitsSeen);
        }
